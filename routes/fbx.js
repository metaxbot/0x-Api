    ...
    res.json({
        ...
        endpoints: [
            ...
            },
            {
                ...
                method: "GET",
                endpoint: "/dl",
                params: [{ name: "url", type: "string", required: true, description: "Facebook Video or Reel URL" }],
                example: `${base}/dl?url=https://www.facebook.com/watch/?v=123456789`
            }
        ]
    });
});
...
// ─────────────────────────────────────────────
// GET /api/fb/dl?url= (ভিডিও ডাউনলোডার)
// ─────────────────────────────────────────────
router.get("/dl", async (req, res) => {
    ...
    if (!videoUrl) {
        return res.status(400).json({ 
            status: false, 
            message: "URL required. Example: /api/fb/dl?url=VIDEO_URL" 
        });
    }
    ...
    try {
        const { data: html } = await axios.get(videoUrl, {
            headers: {
                "User-Agent": randomUA(), // আপনার বিদ্যমান randomUA ফাংশনটি এখানেও ব্যবহার করা হলো
        ...
        // সব সম্ভাব্য ভিডিও কী থেকে লিংক বের করা
        ...
        // ফ্যালব্যাক: ভিডিও ট্যাগ থেকে src
        if (links.length === 0) {
